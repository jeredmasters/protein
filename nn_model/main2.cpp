...
int main2(void)
{
	try
	{
		...
		if (rank == 0)
		{
			...
			char buf[8];			
			int size = variables_pointer->count_unused_variables_number() + variables_pointer->count_used_variables_number();
			for (int i = 0; i < size - 1; i++) {
				sprintf_s(buf, "gene_%d", i);
				variables_pointer->set_name(i, buf);
			}
			variables_pointer->set_name(size - 1, "fitness");

			int inputs = variables_pointer->count_inputs_number();
			int targets = variables_pointer->count_targets_number();
			...
			simulated_annealing_order_pointer->set_maximum_iterations_number(30);
		}
